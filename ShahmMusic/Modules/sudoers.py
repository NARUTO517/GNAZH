...
async def sudoadd(_, message: Message):
    ...
    try:
        ...
        return await message.reply_text("فشلت في اضافة المستخدم الى المطورين .")


@app.on_message(filters.command(["delsudo", "rmsudo"]) | filters.command(["تك","تنزيل","تنزيل مطور","انزل"],prefixes= ["/", "!","","#"]) & filters.user(OWNER_ID))
